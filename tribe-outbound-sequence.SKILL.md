...
**SAY WHICH LANE THE LEAD IS IN FIRST, because the same evidence means opposite things in each (added 25 Aug 2026).** `screen.touch_type` is `first`, `followup` or `second_name`, and `screen.this_contact_status` is the lead status of the person being written to, as distinct from the list of everyone on the account.

This exists because the gate shipped knowing only one lane. Every check in it was written for a cold first touch and then pointed at follow-up sends without anyone re-reading it, and two of them made follow-ups impossible: the worked-status screen blocks any account carrying ATTEMPTED_TO_CONTACT, and this file sets exactly that status on everyone emailed at the moment of sending. So follow-up 1 blocked on the evidence that the first email had been sent. Between that and a `touches_spent >= 2` rule quoting the ladder replaced the same morning, the gate blocked all 58 follow-ups on the calendar through 15 September, and its own selftest read green throughout because all eleven of its cases were first touches. **A gate that only knows one lane fails the other lane silently, so the selftest now carries the follow-up lane and the routing lane too.**

What each lane means to the screen:

| Lane | Use it when | The screen inverts |
|---|---|---|
| `first` | A cold or warm open on a name nobody here has emailed | Anyone on the account at a live status blocks it. This is the amber check. |
| `followup` | Rungs 2, 3 and 4 to a name already on the ladder | This person MUST be at a live status. A follow-up to someone never emailed is a cold email wearing the wrong opener. `touches_spent` must be 1 to 3, and `last_touch_date` must exist, because the seven-day floor cannot be checked against a date nobody recorded. **No connect note is required and none should be invented:** the request went out at touch 1 and is accepted or pending, and withdrawing and resending reads as pestering. |
| `second_name` | The +39 route to a different person at a worked account | Other contacts being live is the whole point. This person being live is not, and blocks. `touches_spent` is 0 and `last_touch_date` is null, because they belong to the person, not the account, and this person has never been emailed. |

**What counts as a touch, once, because two blocking checks key off it.** A touch is an EMAIL SENT TO THAT PERSON. Nothing else is. Not a LinkedIn connect request, not the acceptance message, not a comment on their post. Those are a second CHANNEL, mandated on the same day as touch 1 on purpose, and a channel is not a rung. Count the connect note as a touch and every double-channel lead burns two rungs on day one, so follow-up 1 blocks; count nothing and the seven-day floor stops meaning anything. It follows that `last_touch_date` is the date of the last EMAIL to that person, and **the seven-day floor is a floor between emails, so the same-day connect note does not breach it and never did.**
...
1. **Contact-level prior contact**, which is the check that would have caught amber. `search_crm_objects(CONTACT, associatedWith company X, properties [hs_lead_status, notes_last_contacted, hubspot_owner_id])`. Anything at ATTEMPTED_TO_CONTACT, IN_PROGRESS, CONNECTED, OPEN_DEAL, BAD_TIMING or UNQUALIFIED means that person is already in a conversation with us. Record the whole list AND the status of the person being written to, because the lane decides which of those two facts is disqualifying. **The company record is not enough and never was:** amber's company record had a blank last-contacted while its CEO sat at ATTEMPTED_TO_CONTACT from the previous afternoon.
...
**Do not pick the date by hand.** `python3 scripts/followup_ladder.py <send date> --company=X --variant=A --channel=both` prints the whole ladder with real dates and the task body for each step: bounce check, connect check, DM review, then the three weekly follow-ups Jacopo asked for on 25 Aug 2026 ("I want 3 follow up after the email that we sent... usually after 1 week and then after another week"), then route-or-park and the park review. **The gaps are deliberately not repeated here.** They live in `STEPS` at the top of that script and nowhere else, because the last time this file carried its own copy of them the copy outlived the rule by five days. `--check <proposed> <sent>` says whether a date someone typed is inside the window and why not if it is not.
...
**The rungs and the gaps are not written down here.** `python3 scripts/followup_ladder.py <send date>` prints them with real dates. This paragraph used to restate them, drifted from the ladder the day the cadence changed, and was still being read as current five days later by the sibling skill, which pointed at it by name. One authority, and it is the script.

**Touch 1, the first email.** On confirmed send (verified in Sent, never from the plan): close the send task, rewrite its body with what actually went out (address, subject, variant, date, edits made), and create every downstream task the ladder prints, in one motion. The follow-up tasks each carry the promised give (the index cut), the instruction to re-scan their board first, and "void if replied, close with outcome".

**Follow-ups 1, 2 and 3.** Same motion each time: close the task with the log of what actually went out, and the next rung already exists because it was created at touch 1. Each one must carry something the last did not, which is the whole reason the gap is a week rather than a day. Four emails of the same email is not persistence.

**After follow-up 3.** The route to a second name at the company, sent openly ("if this sits better with X, tell me and I will keep it with her"), or, when no second name makes sense, a PARK task with a dated reopen condition (a new funding round, a new stuck role crossing 90 days, a recruiter posting). Log that lead as `touch_type: second_name` so the gate screens the new person rather than the account. The second person gets the same four rungs. When both names are spent with no reply the account is cold: park it with the reopen condition, close everything else on it, and say so in the report. No third names, no "one last try", and never a fifth email to one inbox.
...
**Sign:** `Best, Jacopo` in English, `Un saluto, Jacopo` in Italian, ALWAYS followed by the signature block **inside the same `htmlBody`**. Drafts created through the Gmail API do not get the Gmail auto-signature, because Gmail only appends that in the UI compose window and never to an existing draft. This is why Jacopo was opening a fresh compose window and pasting the text across by hand: not a preference, a workaround for drafts that arrived without it.

**THERE IS NO LOGO. Verified 25 Aug 2026 against his real sent mail** (the 24 Aug DeepL send, kutylowski@deepl.com). This file used to say the block ended with "the Tribe logo, which Jacopo pastes in manually before sending", and that was simply false: his sent signature is three lines of grey Verdana text and no image at all. A false claim about a manual step is worse than a missing rule, because it tells whoever reads it that the incomplete draft is finished and the human will handle the rest. Nothing needs pasting. Copy this block verbatim into every draft:

```html
<p style="font-family:Verdana,sans-serif;font-size:9pt;color:#7a7a7a">Jacopo Lupo Ferrari<br><a href="https://tribe.xyz/">Head of Delivery @ Tribe.xyz</a><br><a href="https://calendar.google.com/calendar/u/0/appointments/schedules/AcZssZ0ucyiz8Z7z9UGvfNyNXeEh6YJjDHs8Dk02uscgg0swol9OVf5cpWu8u9tSpWTLPU5_AvajhK3y">Book a meeting with me!</a></p>
```

The gate checks for it. On 25 Aug five of six live drafts had no signature at all (Enpal, Dust, Multiverse, Photoroom, Upvest) while the rule saying they must had been written down since 20 August, which is the same shape as every other failure in this file: a real rule with nothing verifying it.
...
- **Sending email is a RULE here, not a limit (corrected 25 Aug 2026).** The Gmail connector exposes send, reply and forward. Jacopo's standing instruction is that nothing goes out without him ("wait always for me before sending anything"), so the desk drafts and he clicks send. State it that way, because this file said flatly that the connector could not send, the desk file said correctly that it could, and a capability claim that is false gets discovered as false; the last time that happened on this portal it cost an afternoon. Gmail requires a recipient, so use `jacopo@tribe.xyz` as a placeholder rather than a guessed prospect address, and open the body with a bracketed line naming the real recipient. **Every draft, placeholder or not, carries `146748263@bcc.eu1.hubspot.com` in the BCC field already**, exactly that string, checked character for character, because a mistyped BCC means the send never reaches HubSpot and the CRM goes blind on the thread. The only edit at send time is the To field.
