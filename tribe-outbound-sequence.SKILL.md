...
4. **Funding rounds: daily sweep plus weekly hunting grounds.** Every daily run sweeps EU funding announcements from the previous day, and every Monday harvest reads the weekly roundup pages as a systematic hunting ground: EU-Startups' weekly funding overview, Tech.eu's weekly roundup, Sifted's coverage. The Flip send proved the daily pattern: round announced the 19th, email referencing it sent the 20th, no cold email is warmer than "your round closed yesterday". The weekly pages catch what the daily sweep missed, but a round older than a week has already been pitched by every agency in Europe, so weekly finds get the board-data opener, not the funding opener.
5. **Company events via the enrichment connector.** Vibe/Explorium's business-events tools can surface hiring-growth and funding events for tracked companies; use it to pre-qualify before the board scan when available.
