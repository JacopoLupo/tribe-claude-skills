...
**The double-channel connect (Jacopo's play, 20 Aug 2026).** On the same day a cold email goes out to a FOUNDER, send a LinkedIn connection request carrying a short note. The note does three things in under 300 characters (the LinkedIn cap, count it before handing the text over): it names the email openly ("I just sent you an email so this is the second knock"), it says one SPECIFIC thing about what they build, and it asks to connect without pitching. Two channels arriving together read as one person being persistent, which is human; two channels pretending not to know about each other read as a sequence, which is software. The specific detail is the whole engine here: "I have been following Gravis since the KIBAG handover video" works because it proves he looked, while "I admire what you do" is flattery any bot can generate and lands as exactly that. Never write generic admiration, find the real detail in their posts, their press or their product. When an email BOUNCES, the connect note becomes the live channel and says so plainly ("I tried reaching you by email and it bounced"). Claude drafts the notes; Jacopo sends them, same as every other channel. When a connect is accepted and the person engages there, touch 2 moves to a LinkedIn DM instead of email, they chose the channel.
