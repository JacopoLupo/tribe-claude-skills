...
| `followup` | Rungs 2, 3 and 4 to a name already on the ladder | This person MUST be at a live status. A follow-up to someone never emailed is a cold email wearing the wrong opener. `touches_spent` must be 1 to 3, and `last_touch_date` must exist, because the seven-day floor cannot be checked against a date nobody recorded. |
